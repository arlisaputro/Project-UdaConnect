...
### config db 
1. to set up environment variables,  run `kubectl apply -f deployment/db-configmap.yaml`
2. to set up secrets, run `kubectl apply -f deployment/db-secret.yaml`
3. to set up a Postgres database running PostGIS, run `kubectl apply -f deployment/postgres.yaml` 
...
2. to check, run  `helm repo list`
3. to install kafka helm, run `helm install udaconnect-kafka bitnami/kafka`
4. to check kafka pod , run kubectl get pod
...
### config api 
1. to set up the persons API, run `kubectl apply -f deployment/1persons-api.yaml`
2. to set up connection, run  `kubectl apply -f deployment/2connections-api.yaml` 
3. to set up env kafka, run `kubectl apply -f deployment/3kafka-configmap.yaml`
4. to set up the location ingester, run `kubectl apply -f deployment/4location-ingester.yaml` 
5. to set up the location service, run `kubectl apply -f deployment/5location-service.yaml`


### config apps
1. to set up frontend app, run `kubectl apply -f deployment/udaconnect-app.yaml` - 
...
### Verifying it Works
1. check pod , run `kubectl get pods` 
2. check service , run  `kubectl get services` \
3. check page running , access web browser 
	a. frontend : `http://localhost:30000/`
	b. all api : `http://localhost:30001/`
	c. person api : `http://localhost:30002/`
	d. connection api : `http://localhost:30003/`
